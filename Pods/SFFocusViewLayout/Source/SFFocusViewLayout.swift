...
open class SFFocusViewLayout: UICollectionViewLayout {
    ...
    fileprivate let constantCategory = 2

    /// Return the size of all the content in the collection view
    override open var collectionViewContentSize : CGSize {
        let contentHeight = CGFloat(numberOfItems) * dragOffset + (height - dragOffset) - (CGFloat(constantCategory) * dragOffset)
    ...
    override open func prepare() {
        ...
        for item in 0..<numberOfItems {
            ...
            {
                if item >= numberOfItems - constantCategory{
